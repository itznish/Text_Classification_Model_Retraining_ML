...
files_list = []
for root, dirs, files in os.walk("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\Training\\"):    #PDF documents filepath
    for file in files:
...
#Exporting to excel
pd.DataFrame(file_content).to_excel("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\Training\\Training_data.xlsx")  #Export to excel file to create training data (specify path) first time run only


# In[56]:
#files_list=[]
#for root, dirs, files in os.walk("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\Retraining\\"):   #filepath for retrained data (training data + feedback data).
#    for file in files:
...
text_data_df = pd.read_excel("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\Training\\Training_data_label.xlsx", index_col=[0])
...
stop_words_full = pd.read_excel("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\ML Resources\\stop_words_full.xlsx") #Stop words file path (extracted from web)
stop_words_full_list = [i for i in stop_words_full['stop_words']]

stop_words = set(stop_words_full_list + stopwords.words('english'))

#Geo-words
geo_words = pd.read_excel("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\ML Resources\\geo_list.xlsx")  #Geo specific stopwords filepath
...
#     text_lemma = ""
    ...
    for token , tag in pos_tag(text_tokens):
        lemma.append(wn.lemmatize(token, tag_map[tag[0]]))
...
Section_keywords = pd.read_excel("E:\\DOCS\\bakup\\freyr\\python_automation_code_package\\ML Resources\\M3_Keywords_32S_individual_words_32S7.xlsx") #Keywords filepath
...
for token , tag in pos_tag(keyword_list):
        keyword_lemma.append(wn.lemmatize(token, tag_map[tag[0]]))
...
import pickle
rand_num = np.random.randint(10000,100000)
root_path = "E:\DOCS\bakup\freyr\python_automation_code_package\Trained_Models" #Rootpath for storing vocabulary
pickle.dump(count_vectorizer.vocabulary_,open(root_path + "vocab_" + str(rand_num) + ".pkl","wb")) #Filepath to save Vocabulary
...
rand_num_model = np.random.randint(1000,10000)
filename = "E:\DOCS\bakup\freyr\python_automation_code_package\Trained_Models" + "NB_Model_" + str(rand_num_model) + ".mdl"   #Specify path to save the trained model file
#print(filename)
...
testfiles=[]
for root, dirs, files in os.walk(""):   #Specify Test files directory path 
    for file in files:
        if file.endswith(".pdf"):
